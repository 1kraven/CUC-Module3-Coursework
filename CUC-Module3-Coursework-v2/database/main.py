...
def populate_fields(event):
    ...
    if selected_row:
        ...
        studentname_ent.insert(0, values[1])
        studentid_ent.delete(0, tk.END)
        studentid_ent.insert(0, values[0])
...
def update_data():
    ...
    if selected_row:
        ...
        cursor.execute(update, (studentname.get(), int(studentid.get()), gender.get(), dob.get(), address.get(), contactnumber.get(), email.get(), course.get(), int(attendance.get()), values[0]))
        connect.commit()
        connect.close()
        fetch_data()
        clear_fields()
...
add_btn = tk.Button(btn_frame,bg="lightgrey",text="Add",bd=7,font=("Calibri"),width=15,cursor='hand2',command=add_func)
add_btn.grid(row=0,column=0,padx=2,pady=2)

update_btn = tk.Button(btn_frame,bg="lightgrey",text="Update",bd=7,font=("Calibri"),width=15,command=update_data)
update_btn.grid(row=0,column=1,padx=3,pady=2)

delete_btn = tk.Button(btn_frame, bg="lightgrey", text="Delete", bd=7, font=("Calibri"), width=15, cursor='hand2', command=delete_func)
delete_btn.grid(row=1,column=0,padx=2,pady=2)

clear_btn = tk.Button(btn_frame,bg="lightgrey",text="Clear",bd=7,font=("Calibri"),width=15)
...
search_in = ttk.Combobox(search_frame,font=("Calibri",14),state="readonly",textvariable=search_by)
search_in["values"] = ("Student Name", "Student ID", "Gender", "Date of birth", "Address", "Contact Phone No.", "Email", "Course", "Attendance")
search_in.grid(row=0,column=1,padx=12,pady=2)

search_btn = tk.Button(search_frame,text="Search",font=("Calibri",13),bd=9,width=14,bg="lightgrey")
...
student_table.column("Attendance",width=120)
