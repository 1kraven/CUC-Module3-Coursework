...
# Define the Enrollment class with methods to add, update, delete and print enrollment details

# add 'StudentID', 'StudentName', 'Registration_Date', 'CourseName', 'CourseID' and anything related to enrollment 


class Enrollment:
    def __init__(self, enrollment_id, enrollment_name):
        self.enrollment_id = enrollment_id
        self.enrollment_name = enrollment_name

    def add_new_enrollment(self, cursor, connection):
        cursor.execute()
        connection.commit()
        self.print_enrollment_details()

    def update_enrollment(self, cursor, connection):
        cursor.execute()
        connection.commit()
        self.print_enrollment_details()
    ...
    @classmethod
    def delete_enrollment(cls, enrollment_id, cursor, connection):
        cursor.execute()
        connection.commit()
        print()

    @classmethod
    def print_all_enrollment_details(cls, cursor, connection):
        cursor.execute()
        rows = cursor.fetchall()
        for row in rows:
            e = Enrollment(row[0], row[1])
